 ...
 * @LastEditTime: 2024-03-01 11:02:11
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
    ...
    if(huart->Instance == USART1) {
        printf("UART1 Call Back\r\n");
        // 接受消息回显到串口助手
        // printf("%s\r\n",receive_buff);          

        // 接受从UART1来的信息后，通过UART2将消息发送到esp8266
        HAL_UART_Transmit_IT(&huart2, (uint8_t*)receive_buff, sizeof(receive_buff));    
        
        HAL_UART_Receive_IT(&huart1, (uint8_t*)receive_buff, sizeof(receive_buff));
    }
    else if(huart->Instance == USART2) {
        printf("UART2 Call Back\r\n");      

        // 接受从UART_Rx 来的信息, 后通过UART1 回显到串口助手
        // printf("%s\r\n",receive_buff);
        HAL_UART_Transmit_IT(&huart1, (uint8_t*)receive_buff, sizeof(receive_buff));
